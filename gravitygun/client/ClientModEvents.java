...
import com.zerokg2004.gravitygun.gravitygun.client.gui.GravityGunConfigScreen;
...
public class ClientModEvents {
    ...
    public static void onClientSetup(FMLClientSetupEvent event) {
        event.enqueueWork(() -> {
            ModLoadingContext.get().registerExtensionPoint(
                    ...
                    () -> new ConfigScreenHandler.ConfigScreenFactory((minecraft, screen) -> new GravityGunConfigScreen(screen))
            );
            ...
            registerRenderModePredicate(ModItems.GRAVITY_GUN_WHITE.get());
            registerRenderModePredicate(ModItems.GRAVITY_GUN_CYAN.get());

            registerRenderModePredicate(ModItems.RED_SUPERCHARGED_GRAVITY_GUN.get());
            registerRenderModePredicate(ModItems.BLUE_SUPERCHARGED_GRAVITY_GUN.get());
            registerRenderModePredicate(ModItems.GREEN_SUPERCHARGED_GRAVITY_GUN.get());
            registerRenderModePredicate(ModItems.ORANGE_SUPERCHARGED_GRAVITY_GUN.get());
            registerRenderModePredicate(ModItems.YELLOW_SUPERCHARGED_GRAVITY_GUN.get());
            registerRenderModePredicate(ModItems.PURPLE_SUPERCHARGED_GRAVITY_GUN.get());
            registerRenderModePredicate(ModItems.WHITE_SUPERCHARGED_GRAVITY_GUN.get());
            registerRenderModePredicate(ModItems.CYAN_SUPERCHARGED_GRAVITY_GUN.get());
        });
    }
    ...
    private static void registerRenderModePredicate(net.minecraft.world.item.Item item) {
        ItemProperties.register(item, RENDER_MODE, (stack, world, entity, seed) -> {
            ...
            if (RenderModeHandler.isSimple()) return 1f;
            if (RenderModeHandler.isDetailed()) return 2f;
            return 2f;
        });
    }
    ...
    public static class ForgeEvents {
        ...
        private static SoundInstance LOOP_SOUND;
        private static boolean lastLoopWasSupercharged = false;
        private static long nextLoopSoundTick = -1L;
        ...
        private static float getLoopPitch(ItemStack stack) {
            return stack.getItem() instanceof com.zerokg2004.gravitygun.gravitygun.item.SuperchargedGravityGunItem ? 0.7F : 1.0F;
        }
        ...
        public static void onClientTick(TickEvent.ClientTickEvent event) {
            ...
            // holding state
            boolean holding = HeldObjectTracker.isHoldingClient(player.getUUID());
            ...
            if (!dual && holding) {
                boolean currentIsSupercharged =
                        currentGun.getItem() instanceof com.zerokg2004.gravitygun.gravitygun.item.SuperchargedGravityGunItem;
                float loopPitch = getLoopPitch(currentGun);
                int loopInterval = currentIsSupercharged ? 69 : 49;

                if (LOOP_SOUND != null && lastLoopWasSupercharged != currentIsSupercharged) {
                    mc.getSoundManager().stop(LOOP_SOUND);
                    LOOP_SOUND = null;
                    nextLoopSoundTick = now;
                }

                if (currentIsGun && (nextLoopSoundTick < 0L || now >= nextLoopSoundTick)) {
                    LOOP_SOUND = SimpleSoundInstance.forUI(
                            currentIsSupercharged ? SoundEventsRegistry.SUPERPHYS_HOLD_LOOP.get() : SoundEventsRegistry.HOLD_LOOP.get(),
                            loopPitch
                    );
                    mc.getSoundManager().play(LOOP_SOUND);
                    lastLoopWasSupercharged = currentIsSupercharged;
                    nextLoopSoundTick = now + loopInterval;
                }
                ...
                lastLoopWasSupercharged = false;
                nextLoopSoundTick = -1L;
            }
        }
