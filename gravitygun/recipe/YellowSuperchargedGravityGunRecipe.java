...
public class YellowSuperchargedGravityGunRecipe extends CustomRecipe {
    private static final String BRUSH_ID = "paintball:yellow_paintbrush";

    public YellowSuperchargedGravityGunRecipe(ResourceLocation id, CraftingBookCategory category) {
        super(id, category);
    }

    @Override
    public boolean matches(CraftingContainer inv, Level level) {
        int superGuns = 0; int brushes = 0;
        for (int i = 0; i < inv.getContainerSize(); i++) {
            ItemStack stack = inv.getItem(i);
            if (stack.isEmpty()) continue;
            if (stack.getItem() instanceof SuperchargedGravityGunItem) superGuns++;
            else {
    ...
    public ItemStack assemble(CraftingContainer inv, RegistryAccess registryAccess) {
        return new ItemStack(ModItems.YELLOW_SUPERCHARGED_GRAVITY_GUN.get());
    }
    ...
    @Override
    public RecipeSerializer<?> getSerializer() { return ModRecipes.YELLOW_SUPERCHARGED_GRAVITY_GUN_RECIPE.get(); }
}